...
class MerchandisingSummaryScreen extends StatefulWidget {
  ...
  @override
  State<MerchandisingSummaryScreen> createState() =>
      _MerchandisingSummaryScreenState();
}

class _MerchandisingSummaryScreenState
    extends State<MerchandisingSummaryScreen> {
  @override
  void initState() {
    super.initState();
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                            ...
                            child: Center(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    provider.buyerWiseValueModel!.summary![0]
                                        .costingCount
                                        .toString(),
                                    style: AppConstants.customTextStyle(
                                        14, Colors.white, FontWeight.normal),
                ...
                const Text("📊 Buyer wise order quantity (Last 30 days)",
                    style: TextStyle(fontWeight: FontWeight.bold)),
                BuyerQtyPieChart(
                    items: provider.buyerWiseValueModel?.buyerWiseQty ?? []),
                const SizedBox(height: 24),
                const Text("📊 Item wise sales value (Last 30 days)",
                    style: TextStyle(fontWeight: FontWeight.bold)),
                ItemWiseSalesChart(
                    items: provider.buyerWiseValueModel?.itemWiseWise ?? []),
                const SizedBox(height: 24),
                const Text("📈 Monthly Order vs Shipment (Line Chart)",
                    style: TextStyle(fontWeight: FontWeight.bold)),
                OrderVsShipmentChart(
                    morrisLine: provider.buyerWiseValueModel!.morrisLine ?? []),
                const SizedBox(height: 24),
                ...
                SizedBox(
                    ...
                    child: BarChartExample(
                        items:
                            provider.buyerWiseValueModel!.buyerWiseQty ?? [])),
              ],
            ),
          );
        },
