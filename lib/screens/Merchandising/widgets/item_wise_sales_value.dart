...
class ItemWiseSalesChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final sortedItems = [...items]..sort((a, b) => (b.quantity ?? 0).compareTo(a.quantity ?? 0));
    final total = items.fold<double>(0, (sum, e) => sum + (e.quantity ?? 0));
    final topBuyer = sortedItems.first;
    final topPercent = ((topBuyer.quantity ?? 0) / total * 100).toStringAsFixed(2);
        ...
        Stack(
          ...
          children: [
            ...
            Column(
              ...
              children: [
                Text(topBuyer.item ?? '', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                Text("$topPercent%", style: const TextStyle(fontSize: 16, color: Colors.grey)),
              ],
            ),
          ],
        ),
          ...
          return Padding(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(item.item ?? '-', style: const TextStyle(fontSize: 14)),
                ),
