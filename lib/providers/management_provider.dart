...
class ManagementProvider extends ChangeNotifier {
  ...
  ManagementDashboardModel? get managementDashboardData =>
      _managementDashboardData;

  Future<void> getAllProductionStatInfo() async {
    var data = await apiService
        .getData2('${AppConstants.baseUrl}api/Dashboard/ProductionDashBoard');
    if (data != null) {
      _managementDashboardData =
          ManagementDashboardModel.fromJson(data['returnvalue']);
      //debugPrint('_managementDashboardData ${_managementDashboardData!.toJson()}');
    }

    debugPrint(
        '_managementDashboardData ${_managementDashboardData!.toJson()}');
  ...
  Future<void> getAllDropdownInfoForJobcard() async {
    try {
      ...
      if (data != null) {
        allDropdownInfoForJobcard =
            JobCardDropdownModel.fromJson(data['Result']);
      }
    } catch (e) {
      debugPrint('Error $e');
    } finally {
