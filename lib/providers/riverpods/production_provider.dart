...
final errorSummaryProvider=StateNotifierProvider<ErrorSummaryNotifier,List<ErrorSummaryModel>>((ref){
...
class ErrorSummaryNotifier extends StateNotifier<List<ErrorSummaryModel>>{
  ...
  Future<void> loadErrorSummaryData(String date) async {
    try {
      ...
      final data = await apiService.getData('api/Support/GetDailyErrorSummary?startDate=$date&endDate=$date');
      for(var i in data['data'])
      {
  ...
  int get totalDefects {
    return state.fold(0, (sum, item) => sum + (item.totalDefects?.toInt() ?? 0));
  }
